...
public class Database {
  ...
  Map<String, Integer> getFrequencyMap() throws SQLException {
    ...
    PreparedStatement prep = conn.prepareStatement("SELECT corpus.filename, COUNT(*) FROM corpus, word WHERE corpus.id = word.corpus_id GROUP BY corpus.filename");//Your SQL here!
    // "SELECT corpus.filename word.word COUNT(*) FROM corpus JOIN word ON corpus.id = word.corpus_id "
    //SELECT corpus.filename, COUNT(*) FROM corpus, word WHERE corpus.id = word.corpus_id GROUP BY corpus.filename"
